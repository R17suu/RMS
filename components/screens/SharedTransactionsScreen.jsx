...
import { fetchSalesByClerk, fetchSalesHistory, voidSale } from '../../services/saleService';
import AppButton from '../../components/AppButton';

const FILTERS = ['My Sales', 'All Sales'];

export default function SharedTransactionsScreen({ headerComponent, role = 'admin' }) {
	...
	const totalRevenue = sales.reduce((sum, sale) => sum + (sale.status !== 'Voided' ? (parseFloat(sale.totalAmount) || 0) : 0), 0);
	const totalItemsSold = sales.reduce((sum, sale) => {
		return sum + (sale.status !== 'Voided' ? (sale.items?.reduce((itemSum, item) => itemSum + item.quantity, 0) || 0) : 0);
	}, 0);
	...
	const handleVoidTransaction = async () => {
		...
		setIsVoiding(true);
		try {
			await voidSale(selectedSale.id, auth.currentUser.uid);
			showToast('Transaction successfully voided.', 'success');
				...
				<View style={styles.modalOverlay}>
					<AppCard style={styles.modalCard}>
						...
						<ScrollView style={styles.reviewScroll} showsVerticalScrollIndicator={false}>
							...
							{role === 'admin' && selectedSale.status !== 'Voided' && (
								<View style={{ marginTop: 24, marginBottom: 12 }}>
									<AppButton 
										title={isVoiding ? "Voiding..." : "Void Transaction"} 
										iconName="warning-outline"
										onPress={handleVoidTransaction}
										disabled={isVoiding}
										style={{ backgroundColor: 'rgba(239,68,68,0.15)', borderColor: 'rgba(239,68,68,0.5)', borderWidth: 1 }}
